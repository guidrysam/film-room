...
import {
  gameReviewUrl,
  loadLinkedPlayerProfile,
  type LinkedPlayerProfile,
} from "@/lib/linked-players";
import { myPlayersUrl, teamPlayerProfileUrl } from "@/lib/team-routes";

export type LinkedPlayerProfileViewProps = {
  groupKey: string;
  currentUid: string;
};
...
export default function LinkedPlayerProfileView({
  groupKey,
  currentUid,
}: LinkedPlayerProfileViewProps) {
  const [data, setData] = useState<LinkedPlayerProfile | null>(null);
  ...
  const refresh = useCallback(async () => {
    ...
    setError(null);
    try {
      const profile = await loadLinkedPlayerProfile(currentUid, groupKey);
      if (!profile) {
        setError("Player not found or you are not linked as a parent.");
        setData(null);
        return;
      }
      setData(profile);
  ...
  if (error || !data) {
    return (
      <div className="mx-auto max-w-lg px-4 py-16 text-center">
        ...
        <Link href={myPlayersUrl()} className={`${ghostBtn} mt-4 inline-block`}>
          ← My kids
        </Link>
      </div>
    );
  ...
  const { group, rosterAppearances, filmMoments, eventSummaries, careerCounts, careerTotal } =
    data;
    ...
    <div className="min-h-screen px-4 py-10 text-zinc-50">
      <div className="mx-auto max-w-2xl">
        <div className="mb-6 border-b border-white/[0.06] pb-5">
          ...
          <h2 className="text-xl font-semibold text-white">{group.displayName}</h2>
          <p className="mt-1 text-sm text-zinc-400">
            Tagged plays and stats across every team you are linked to.
          </p>
        </div>
        ...
        <section className={`${panelClass} mb-5`}>
          ...
          {careerTotal > 0 ? (
            <p className="mt-3 text-xs text-zinc-400">
              All events: {formatCareerStatLine(careerCounts)}
            </p>
          ) : null}
        ...
        <section className={`${panelClass} mb-5`}>
          <h2 className="mb-3 text-sm font-semibold text-white">Stats on film</h2>
          {statMoments.length === 0 ? (
            <p className="text-sm text-zinc-400">No stats tagged yet.</p>
          ) : (
            <ul className="space-y-1.5">
        ...
        <section className={panelClass}>
          <h2 className="mb-3 text-sm font-semibold text-white">Tagged moments</h2>
          {tagMoments.length === 0 ? (
            <p className="text-sm text-zinc-400">No tagged moments yet.</p>
          ) : (
            <ul className="space-y-1.5">
        ...
        <Link href={myPlayersUrl()} className={`${ghostBtn} mt-6 inline-block`}>
          ← My kids
        </Link>
      </div>
    </div>
